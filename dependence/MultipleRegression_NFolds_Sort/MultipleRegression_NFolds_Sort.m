function Prediction = MultipleRegression_NFolds_Sort(Subjects_Data, Subjects_Scores, FoldQuantity, Pre_Method,  Permutation_Flag, ResultantFolder)
%
% Subject_Data:
%           m*n matrix
...
%     model = svmtrain(Training_scores, Training_data_final, ['-s 3 -t 0 -c ' num2str(C_Parameter)]);
    % 用 regstats 做回归（等价于 fitlm 也行）
    stats = regstats(Training_scores, Training_data_final, 'linear');
    beta = stats.beta;   % 回归系数 (含截距)
...
%     [Predicted_Scores, ~, ~] = svmpredict(test_score, test_data_final, model);
 % 在测试集上预测
    Predicted_Scores = [ones(size(test_data_final,1),1) test_data_final] * beta;
...
%     [Prediction.Corr_r(j),Prediction.Corr_p(j)] = corr(Predicted_Scores, test_score);
...
SS_res = sum((TrueScore - PreScore).^2);
SS_tot = sum((TrueScore - mean(TrueScore)).^2);
R2_all = 1 - SS_res/SS_tot;
Prediction.R2_all = R2_all;
